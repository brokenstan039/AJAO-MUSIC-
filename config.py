## What's up Kangers

import os
from os import getenv
from dotenv import load_dotenv

if os.path.exists("local.env"):
    load_dotenv("local.env")

load_dotenv()
admins = {}
SESSION_NAME = getenv("SESSION_NAME", "1AZWarzQBuxNHR_5VyZB_B71tOYfa-I95-RWlLWpSXFTbSq1tJ5UlAx-oW6iwhoZzYLPBoNngpFRB7PKEqJxOGKduV_vPCuUoIwnAqpplKqBL7NeE-YYmN-xDGXyJ1UoAyxgctPHlR_Uvjl3xrZQTV9fYhi_zTBMDJDe6WxZiKXiigDFjrjl89LA0yXqISDudhTmaeiyqFFDJf7A_FIN3lqsEKsS2itpGg6tHjRPunE0zcPC9mkZanMFQYUGbxWVA7cGcfJd_XTSHjlme-3mBRddRer4TYoaFeWDLA_lfAJObJ64R4WRDGAeT13sAAaTdfzMmyYNzwiAu1orcqLH6WYBcb_x8_GM=")
BOT_TOKEN = getenv("BOT_TOKEN", "5731586246:AAFJ5SDMF1peb_c3hhT85i7qrkTF8hH1xC0")
BOT_NAME = getenv("BOT_NAME", "F4R X MUSIX")
API_ID = int(getenv("API_ID", "23785459"))
API_HASH = getenv("API_HASH", "4ad74412e0cff337b063c655bd72807d")
OWNER_NAME = getenv("OWNER_NAME", "💍𝐃𝐄𝐒𝐓𝐑𝐔𝐂𝐓𝐄𝐑💍𝐓𝐈𝐓𝐀𝐍💍")
OWNER_USERNAME = getenv("OWNER_USERNAME", "DESTRUCTER_TITAN")
ALIVE_NAME = getenv("ALIVE_NAME", "TITAN")
BOT_USERNAME = getenv("BOT_USERNAME", "F4R_X_MUSIC_BOT")
OWNER_ID = getenv("OWNER_ID", "5912734719")
ASSISTANT_NAME = getenv("ASSISTANT_NAME", "DESTRUCTER_TITAN")
GROUP_SUPPORT = getenv("GROUP_SUPPORT", "TheSupportChat")
UPDATES_CHANNEL = getenv("UPDATES_CHANNEL", "TheUpdatesChannel")
HEROKU_APP_NAME = getenv("HEROKU_APP_NAME")
HEROKU_API_KEY = getenv("UPDATES_CHANNEL", "HEROKU_API_KEY")
SUDO_USERS = list(map(int, getenv("SUDO_USERS", "5912734719").split()))
COMMAND_PREFIXES = list(getenv("COMMAND_PREFIXES", "/ ! .").split())
ALIVE_IMG = getenv("ALIVE_IMG", "https://telegra.ph/file/fc9d87ffd1c6f828eb7fc.png")
START_PIC = getenv("START_PIC", "https://telegra.ph/file/a414e2cdfeaa7d4414b89.png")
DURATION_LIMIT = int(getenv("DURATION_LIMIT", "60"))
UPSTREAM_REPO = getenv("UPSTREAM_REPO", "https://github.com/ITZ-ZAID/Zaid-Vc-Player")
IMG_1 = getenv("IMG_1", "https://telegra.ph/file/c540aac0249486854787b.png")
IMG_2 = getenv("IMG_2", "https://telegra.ph/file/6f1cfec700087f6fcb391.png")
IMG_3 = getenv("IMG_3", "https://telegra.ph/file/c3547532105a0cb67229d.png")
IMG_4 = getenv("IMG_4", "https://telegra.ph/file/be5f551acb116292d15ec.png")
IMG_5 = getenv("IMG_5", "https://telegra.ph/file/c3401a572375b569138c3.png")
IMG_6 = getenv("IMG_6", "https://telegra.ph/file/c3401a572375b569138c3.png")
